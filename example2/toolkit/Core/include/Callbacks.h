...
extern "C"
{
	...
	typedef void (OTAPI_CALL *CB_OTTraceOnLogMessage)(CB_OnLogMessageCore pApiCallback,
		...
		const TCHAR* traceLevel,
		OTUInt32 threadId,
		const TCHAR* traceGroup,
	...
	typedef EnumStatusCode (OTAPI_CALL *CB_OTBApplicationHandleMonitoringChange)(
			TBHandle otbHandle,
			unsigned NodesCount, TBHandle* Nodes,
			unsigned ChangesCount, MonitoringChangeStruct* pChanges);
	...
	typedef void (OTAPI_CALL *CB_OTSEndpointOnSecureChannelOpened)(
			...
			const TCHAR* secureChannelId,
			const ByteStringStruct* clientCertificate,
			const TCHAR* clientCertificateThumbprint,
	...
	typedef void (OTAPI_CALL *CB_OTSSessionOnTransactionCancelled)(
		TBHandle otbHandle, EnumStatusCode result, OTUInt32 transactionId, const TCHAR * auditEntryId);
