...
namespace SoftingOPCToolbox5
{
	...
	namespace Server
	{
		...
		public:
			...
			/*! @deprecated		Please use Condition::create() to construct an instance of this type.
		...
		public:
			...
			*		can be retrieved by TwoStateVariable::getTrueState() and TwoStateVariable::getFalseState(). */
			...
			/*! @deprecated	Please do not call this function directly.
			*				Use getEnabledStateVar()->setTrueState() and getEnabledStateVar()->setFalseState()
			*				to define the localized text for the Enabled and Disabled states.
			*				Use Condition::enable() or Condition::disable() to enable or disable the condition. */
			DEPRECATED(virtual EnumStatusCode setEnabledState(const ILocalizedText* enabledState));
			/*! @overload */
			DEPRECATED(virtual EnumStatusCode setEnabledState(const ILocalizedText& enabledState));
			...
			* @param conditionName ConditionName identifies the Condition instance that the Event originated from. It can be used together with
			...
			EnumStatusCode getConditionName(tstring &conditionName);
