	...
	namespace Server
	{
		...
		* If the server application does not want these components, it can remove them after calling init(). */
		...
		public:
			...
			/*! @deprecated		Please use AlarmCondition::create() to construct an instance of this type.
		...
		public:
			...
			*       can be retrieved by TwoStateVariable::getTrueState() and TwoStateVariable::getFalseState(). */
			...
			/*! @deprecated	Please use TwoStateVariable::setCurrentState() to change the current
			*				active state (see AlarmCondition::getActiveStateVar()).
			*				The localized texts that describe the 'true' and 'false' active state
			*				can be defined by TwoStateVariable::setTrueState() and TwoStateVariable::setFalseState(). */
			DEPRECATED(virtual EnumStatusCode setActiveState(const ILocalizedText* activeState));
			/*! @overload */
			DEPRECATED(virtual EnumStatusCode setActiveState(const ILocalizedText& activeState));
			...
			* alarms. */
			...
			/*! @deprecated Please use AlarmCondition::suppress() instead. */
			DEPRECATED(virtual EnumStatusCode setSuppressedOrShelved(bool suppressedOrShelved));
			...
			*       can be retrieved by getSuppressedStateVar()->getTrueState() and getSuppressedStateVar()->getFalseState(). */
			...
			/*! @deprecated	Please use AlarmCondition::suppress() to change the current suppression state.
			*				The localized texts that describe the 'true' and 'false' suppression state
			*				can be defined by calling TwoStateVariable::setTrueState() and
			*				TwoStateVariable::setFalseState() on the SupressedState variable which can
			*				be retrieved using AlarmCondition::getSuppressedStateVar(). */
			DEPRECATED(virtual EnumStatusCode setSuppressedState(const ILocalizedText* suppressedState));
			/*! @overload */
			DEPRECATED(virtual EnumStatusCode setSuppressedState(const ILocalizedText& suppressedState));
