	...
	* The "null" value represents January 1, 1601 00:00:00 (UTC) (see IDateTime::isNull()). */
	...
	public:
		...
		* @param ticks	100 nanosecond ticks since 1601. */
		...
		virtual bool operator==(const IDateTime& other);
		/* see IDateTime::operator!= */
		virtual bool operator!=(const IDateTime& other);
		...
		* @note This method is only available on Microsoft Windows operating systems. */
		...
		/*! @deprecated Please use utcNow() instead.
		*
		* Sets this DateTime instance to the current date and time.
		* This method is an alias for utcNow(). */
		DEPRECATED(void now());

		/* see IDateTime::utcNow */
		virtual void utcNow();

		/*! @deprecated Please use getUtcNow() instead.
		*
		* Returns a DateTime instance containing the current date and time.
		* This method is an alias for getUtcNow(). */
		DEPRECATED(static DateTime getNow());
		...
		* If @p from > @p to, the method returns 0. */
		...
		/* see IDateTime::fromString */
		DEPRECATED(virtual EnumStatusCode fromString(const tstring& dateString));
