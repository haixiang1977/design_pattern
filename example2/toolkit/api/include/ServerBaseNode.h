		...
		* for all nodes classes and cannot be instantiated directly. */
		...
		public:
			...
			* This method is provided for inserting a complete node tree with a single call. */
			...
			/*! @deprecated Please use remove() or removeTree() instead. */
			DEPRECATED(EnumStatusCode remove(bool includeReferences));
...
#if TB5_ALARMS
			/*! @deprecated		This method is deprecated. Please call the report() method on the event you
			*					want to emit instead (see Server::BaseEventNotification::report() and
			*					Server::BaseEvent::report())
			*
			* Reports an event for this node.
			* The function returns an error in case of an invalid configuration of the @p notification object.
			*
			* @note This method will also set following properties of the event:
			*	@li the EventId to a unique value
			*   @li the SourceNode to the NodeId of the node on which the method is called upon
			*   @li the TypeId to the NodeId of the event type
			*
			* Any previous values of these nodes will be overwritten. */
			DEPRECATED(EnumStatusCode reportEvent(Server::EventNotification* notification));
#endif
			...
			* be set, but will not be reported by a subscription. */
			...
			/*! @deprecated Use removeTree only to remove nodes from the address space. */
			...
			*
			* @note		The Toolkit will not touch the user data. The server application is responsible to
			...
			*		more details. */
			...
			EnumStatusCode getExtensions(std::vector<XmlElement>& extensions);
