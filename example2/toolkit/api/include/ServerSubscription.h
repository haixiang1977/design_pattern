		...
		* an OPC UA client to monitor data changes and/or events. */
		...
		public:
			...
			*		in order to revise the value requested by the client. */
			...
			/*! Returns the identifier of the subscription. This identifier has been assigned by the Toolkit core. */
...
#define CTT_1_2_336_276
			...
			* The toolkit can validate this in various service calls and reject duplicates.
			*
			...
			* If this validation is enabled the toolkit will remove the already existing monitored items with identical clientHandle.
			*
			...
			* @li Priority (cannot be revised) */
			...
			/*! Callback that is called when the client deletes the subscription.
			*
			* Note: A subscription may have a longer lifetime than a session.
			* Due to this fact the callbacks for creation and deletion are implemented at the subscription class.
			* Subscription::getSession() may return NULL when called during onDelete().
			*
			* @deprecated Instead of this callback please overwrite the callback handleDelete(). */
			DEPRECATED(virtual EnumStatusCode onDelete());
			...
			* @see handleDeleteMonitoredItems() */
			...
			/*! Callback that is called when the client creates one or more monitored items.
			*
			* This callback is executed before the response to the client is assembled.
			* That allows the server implementation to verify and optionally revise the parameters of the
			* monitored items that are being created.
			*
			* @note Some elements in the monitored items array may be NULL, if creating those elements failed.
			*
			* @deprecated Instead of this callback please overwrite the callback handleCreateMonitoredItems(). */
			DEPRECATED(virtual EnumStatusCode onCreateMonitoredItems(const std::vector<Server::MonitoredItem*>& monitoredItems));

			/*! Callback that is called when the client modifies the parameters of one or more monitored items.
			*
			* This callback is executed before the response to the client is assembled.
			* That allows the server implementation to verify and optionally revise the parameters of the monitored items
			* that are being modified.
			*
			* @note Some elements in the monitored items array may be NULL, if modifying those elements failed.
			*
			* @note The return value is ignored. There is no possibility to reject modification in general.
			* It is only possible to revise modified values (queue size and sampling rate) during processing.
			*
			* @deprecated Instead of this callback please overwrite the callback handleModifyMonitoredItems(). */
			DEPRECATED(virtual EnumStatusCode onModifyMonitoredItems(const std::vector<Server::MonitoredItem*>& monitoredItems));
			/*! Callback that is called when the one or more monitored items are deleted.
			*
			* @note Some elements of the monitored items array parameter may be NULL, if finding those elements failed.
			*
			* @note The return value is ignored. There is no possibility to reject deletion of monitored items
			*
			* @deprecated Instead of this callback please overwrite the callback handleDeleteMonitoredItems(). */
			DEPRECATED(virtual EnumStatusCode onDeleteMonitoredItems(const std::vector<Server::MonitoredItem*>& monitoredItems));
		...
		private:
			...
			/*! Subscriptions are always created by the Toolkit core. */
			Subscription();
