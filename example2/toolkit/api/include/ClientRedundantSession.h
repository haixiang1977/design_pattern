		...
		*		while the session is connected. */
		...
		public:
			...
			* @see	setSecurityConfiguration() */
			...
			/*! Sets both the message security mode and security policy for the redundant session to be established.
			*
			* The message security mode determines whether to encrypt and/or sign messages that are exchanged between
			* the client and the server. The security policy URI defines the encryption and signature algorithms
			* that are used to accomplish the requested message security.
			*
			* The message security mode is closely linked to the security policy. Either both values are set to 'none'
			* or both are set to a value different from 'none'.
			*
			* @param	messageSecurityMode	Desired message security mode
			* @param	securityPolicy		Desired security policy
			*
			* @see	EndpointDescription Application::getEndpointsFromServer() */
			...
			*
			* @note	When there is currently no primary server, the value OTClientRedundantServerIndex_Invalid is
			...
			*
			* If connect() failed the Toolkit cyclically tries to re-establish the connections until disconnect() is called.
			...
			*
			* @note	If an error can't be distributed to a server index, @p redundantServerIndex will return OTClientRedundantServerIndex_Invalid.
			...
			*		@p redundantServerIndex will return OTClientRedundantServerIndex_Invalid. Make sure to not use this special index to specify
			...
			*		OTClientRedundantServerIndex_Invalid. Make sure to not use this special index to specify a single server or
			...
			* @note	If no DataTypeDescription exists for the specified data type, then NULL is returned. */
			...
			/*! @overload
			* @return	Returns the DataTypeDescription from the primary server */
