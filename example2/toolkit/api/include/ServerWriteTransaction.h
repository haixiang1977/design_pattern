...
namespace SoftingOPCToolbox5
{
	...
	namespace Server
	{
		...
		*
		* The Toolkit creates a new transaction object for every service call and triggers the
		...
		* to the service call is sent to the client. */
		...
		public:
			...
			/*! Callback method that is called by the Toolkit whenever a client uses the 'Write' service
			...
			*
			* @note The requests within this transaction are already preprocessed by the toolkit.
			*		If the toolkit has found any errors (e.g. EnumStatusCode_BadNodeIdUnknown, EnumStatusCode_BadNotWritable...)
		...
		private:
			...
			/*! Transactions are always created by the Toolkit core. */
			WriteTransaction();
